...
class DETRsegm(nn.Module):
    ...
    def forward(self, samples: NestedTensor, history_samples: NestedTensor):
        ...
        out["pred_logits"] = (self.detr.class_embed(hs))[-1]
        out["pred_boxes"] = (self.detr.bbox_embed(hs).tanh())[-1]
...
class multi_scale(nn.Module):
    ...
    def forward(self, features, features2, query_embed_weight, pos):
        ...
        hs3, memory3 = self.transformer(t4, mask, hs4[-1].squeeze(0), pos[-2])
        ...
        hs2, memory2 = self.transformer(t3, mask, hs3[-1].squeeze(0), pos[-3])
        ...
        hs1, memory1 = self.transformer(t2, mask, hs2[-1].squeeze(0), pos[-4])
