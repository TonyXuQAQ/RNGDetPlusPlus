...
for tile_idx in tile_list:
    ...
    graph_prop = '../%s/graph/%s_crop.p'%(args.savedir,tile_idx)
    graph_gt = '../data/RGB_1.0_meter/%s__gt_graph_dense_spacenet.p'%tile_idx
    args.output = '../%s/results/topo/%s.txt'%(args.savedir,tile_idx)

    map1 = pickle.load(open(graph_gt, "r"))
    map2 = pickle.load(open(graph_prop, "r"))
