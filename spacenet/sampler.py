...
class Edge(FrozenClass):
    def __init__(self,args,id,src,dst,vertices,orientation,circle):
        self.args = args
        self.id = id
        # init and end
        self.src = src
    ...
    def step_BC(self,init_step=False):
        ...
        if self.circle and init_step:
            edge_move_ahead_length = min(self.args.edge_move_ahead_length,len(self.vertices)//3)
        # find index now
        ...
        if index_now + edge_move_ahead_length >= len(self.vertices) and (not self.circle or not init_step):
    ...
    def load_data_and_initialization(self,tile_name):
        ...
        for i, e in enumerate(edges):
            try:
                ...
                print(e['src'],e['dst'],e)
                raise Exception
            new_edge = Edge(self.args,e['id'],src,dst,e['vertices'],e['orientation'],e['circle'])
            self.edges[e['id']] = new_edge
            src.add_neighbor(new_edge)
            dst.add_neighbor(new_edge)
            pass
